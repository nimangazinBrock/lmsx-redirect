...
csv_path = "C:\ProgramData\MySQL\MySQL Server 5.6\Data\\bds\\"
...
def trade_in_refresh_token(config):
    ...
    if response.status_code != 200:
        print('Status code: %s; content: %s; message: %s', response.status_code, response.text,
...
def unzip_and_update_db(response_content, db_conn_params, table):
    ...
    with io.BytesIO(response_content) as response_stream:
        with zipfile.ZipFile(response_stream) as zipped_data_set:
            zipped_data_set.extractall(csv_path)
...
def date_formatting(db_conn_params, table, csv_file):
    ...
    with open(csv_path+csv_file, "r", encoding='utf-8-sig') as data_file:
...
print("start running Brightspace")
...
print("Brightspace code ended")
print("Start running Sakai jobs")
# Run sakai jobs
sakai.sakai_run(db_conn_params, config)

print("Sakai jobs ended")
# #sakai.delete_all_sites_for_user(config, "nimangazin", config['current_term_eid'])
# sakai.delete_tmp_tables(db_conn_params)